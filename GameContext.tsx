    ...
    const processMissionArrival = async (mission: FleetMission) => {
        ...
        if (mission.type === MissionType.EXPEDITION) {
            try {
                ...
                let result: MissionLog | undefined;
                let loot: any = {};
                let survivingAttacker = { ...mission.ships };

                if (mission.type === MissionType.EXPEDITION) {
                    // ... (Expedition Logic)
                    const expeditionResult = calculateExpeditionOutcome(mission.ships);
                    result = expeditionResult.log;

                    // Add resources/ships from expedition to return cargo
                    if (expeditionResult.resources) loot = expeditionResult.resources;
                    if (expeditionResult.ships) {
                        loot.ships = expeditionResult.ships;
                    }
                    ...
                    if (mission.targetUserId) {
                        ...
                        if (targetProfile) {
                            const battle = generatePvPBattleResult(
                                mission.ships,
                                targetProfile.ships,
                                targetProfile.buildings, // simplistic, assumes buildings are in profile? actually they are separate usually, but here assumed in profile or generic
                                targetProfile.research, // same assumption
                ...
                // Verify if we should fallback status to returning to avoid stuck missions?
                // For now just log.
            }
        };
