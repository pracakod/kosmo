...
const calculatePoints = (resources: any, buildings: any, ships: any, research: any, defenses: any) => {
    // 1. Resources Points (REMOVED: Liquid resources do not count towards score)
    // const resPoints = ((Number(r.metal) || 0) + (Number(r.crystal) || 0) + (Number(r.deuterium) || 0));

    // Helper to calc value with Deuterium x2
    const calcValue = (cost: { metal: number, crystal: number, deuterium: number }) => {
        return (cost.metal || 0) + (cost.crystal || 0) + ((cost.deuterium || 0) * 2);
    };
    ...
    Object.entries(buildings || {}).forEach(([id, level]) => {
        ...
        buildPoints += calcValue({ metal, crystal, deuterium });
    });
    ...
    Object.entries(research || {}).forEach(([id, level]) => {
        ...
        researchPoints += calcValue({ metal, crystal, deuterium });
    });
    ...
    Object.entries(ships || {}).forEach(([id, count]) => {
        ...
        if (!def) return;
        fleetPoints += calcValue(def.baseCost) * (count as number);
    });
    ...
    Object.entries(defenses || {}).forEach(([id, count]) => {
        ...
        if (!def) return;
        defensePoints += calcValue(def.cost) * (count as number);
    });

    const totalValue = buildPoints + researchPoints + fleetPoints + defensePoints;
...
export const GameProvider: React.FC<{ children: ReactNode, session: any }> = ({ children, session }) => {
    ...
    const mainPlanetCacheRef = useRef(mainPlanetCache);

    // Keep ref synchronized
    useEffect(() => {
        gameStateRef.current = gameState;
        currentPlanetRef.current = currentPlanetId;
        mainPlanetCacheRef.current = mainPlanetCache;
    }, [gameState, currentPlanetId, mainPlanetCache]);
    ...
    const saveGame = async (reason: string): Promise<boolean> => {
        ...
        if (isColony) {
            ...
            profilePayload = {
                ...
                xp: current.xp,
                points: (() => {
                    // CALCULATE TOTAL GLOBAL POINTS (Account Wide) - COLONY CONTEXT
                    let totalPoints = 0;

                    // 1. Current State (Colony)
                    // Research is global (in current state), so we count it here.
                    totalPoints += calculatePoints(null, current.buildings, current.ships, current.research, current.defenses);

                    // 2. Main Planet Points (Need to fetch or use cached?)
                    // If we don't have main planet details, we might under-count.
                    // Ideally we should have 'mainPlanetCache' or similar.
                    // For now, iterate 'planets' array? No, 'planets' only contains COLONIES usually if using .eq('owner_id').

                    // Actually, 'fetchPlanets' usually excludes main if it's in 'profiles'. 
                    // Let's check 'planets' content. If it contains only colonies, we are missing Main.
                    // However, we can use 'profileData' we just fetched above?
                    // 'profileData' selects 'resources, debris'. We need buildings/ships etc.

                    // FIX: We need to trust the server to calculate Total Points? 
                    // OR we accept that calculating points on Client Side requires ALL data.

                    // Let's use what we have. If we are on Colony, 'planets' likely contains OTHER colonies.
                    // We are missing Main Planet Data.
                    // We can assume Main Planet Data hasn't changed *on this tick* if we are on Colony.
                    // So we should have cached it when switching?

                    // Assuming 'planets' array contains ALL USER PLANETS (Colonies). Main is Profile.
                    // We need Main Planet Buildings/Ships.
                    // If we can't reliably calculate it here, we shouldn't overwrite 'points' field with a partial sum!

                    // BETTER APPROACH: Don't send 'points' in colony update if we can't calc it?
                    // BUT: rankings depend on 'points'.
                    // Let's fetch Profile Buildings/Ships to be safe.
            ...
            profilePayload = {
                ...
                points: (() => {
                    ...
                    // 1. Current State Points (Main Planet)
                    totalPoints += calculatePoints(null, current.buildings, current.ships, current.research, current.defenses);

                    // 2. Colony Points (From 'planets' array, excluding current if somehow in list)
                    planets.forEach(p => {
                        // Skip if this planet is currently loaded as 'current' (shouldn't happen on Main save, but safe check)
                        if (p.id === currentPlanetId) return;
                        totalPoints += calculatePoints(null, p.buildings, p.ships, {}, p.defenses);
                    });
        ...
        try {
            ...
            if (error) {
                ...
                if (error.message && error.message.includes('VERSION_MISMATCH')) {
                    console.warn('⚠️ Version mismatch, reloading...');
                ...
                if (mission.type === MissionType.COLONIZE) {
                    ...
                    if (!targetObj) {
                        ...
                        if (totalDebris <= 0) {
                            ...
                            // USE RPC FOR SECURE DEBRIS UPDATE
                            const debrisDiff = {
                                metal: -metalToTake,
                                crystal: -crystalToTake
                            };

                            const { error: rpcError } = await supabase.rpc('apply_mission_outcome', {
                                p_mission_id: mission.id,
                                p_target_id: targetObj.id, // ID of Planet or Profile
                                p_is_planet: isPlanet,     // Correct flag
                                p_log_entry: null,         // No log for target needed usually, or add one? Let's skip for now or add "Debris Harvested"?
                                p_resources_diff: null,
                                p_debris_diff: debrisDiff,
                                p_ships_new: null,
                                p_defenses_new: null,
                                p_buildings_new: null
                            });

                            if (rpcError) {
                                console.error("Recycle RPC Error:", rpcError);
                                result = { id: `${mission.id}-err`, timestamp: Date.now(), title: 'Błąd Recyklingu', message: rpcError.message, outcome: 'failure' };
                            } else {
                                // Update Fleet with Resources (Local & DB) 
                                // Fleet update is owned by us, so we can do it directly or via RPC. 
                                // Since we own the fleet, direct update is authorized.
                    ...
                    if (!mission.targetUserId) {
                        ...
                        if (mission.type === MissionType.ATTACK) {
                            ...
                            // XP
                            if (pvpBattleResults.report.result === 'attacker_win') addXP(50, 'PvP Victory');
                            ...
                            // 1. Resolve Tech Levels
                            let attackerResearch: any = {};
                            if (mission.ownerId === session.user.id) {
                            ...
                            // Level >= Target: Resources
                            // Level >= Target + 2: Fleet
                            // Level >= Target + 3: Defense
                            ...
                            result = {
                                ...
                                message: `Skanowanie zakończone. Poziomy: Ty:${attackerSpyLevel} vs Cel:${defenderSpyLevel}. \n${details.join('\n')}`,
                                outcome: 'success',
                                report: spyReport // UI should render this
                            };
                        }

                        // C. SECURE INTERACTION RPC (Target)
                        // Use specialized RPC to allow "Attacker" to safe write to "Defender"

                        // Prepare Diffs
                        const resourcesDiff: any = {};
                        if (mission.type === MissionType.ATTACK) {
                            resourcesDiff.metal = -(loot.metal || 0);
                            resourcesDiff.crystal = -(loot.crystal || 0);
                            resourcesDiff.deuterium = -(loot.deuterium || 0);
                        } else if (mission.type === MissionType.TRANSPORT) {
                            resourcesDiff.metal = (mission.resources?.metal || 0);
                            resourcesDiff.crystal = (mission.resources?.crystal || 0);
                            resourcesDiff.deuterium = (mission.resources?.deuterium || 0);
                        }

                        const debrisDiff: any = {};
                        if (mission.type === MissionType.ATTACK) {
                            const addedDebris = (pvpBattleResults as any)?.debris || { metal: 0, crystal: 0 };
                            debrisDiff.metal = addedDebris.metal;
                            debrisDiff.crystal = addedDebris.crystal;
                        }

                        // Prepare New State (for Attack)
                        // Only pass these if they changed (Attack), otherwise undefined (RPC will keep current)
                        const newShips = mission.type === MissionType.ATTACK ? pvpBattleResults?.survivingDefenderShips : undefined;
                        const newDefenses = mission.type === MissionType.ATTACK ? pvpBattleResults?.survivingDefenderDefenses : undefined;
                        const newBuildings = mission.type === MissionType.ATTACK ? newTargetData.buildings : undefined;
                        ...
                        const { error: rpcError } = await supabase.rpc('apply_mission_outcome', {
                            ...
                            p_debris_diff: debrisDiff,
                            p_ships_new: newShips,
                            p_defenses_new: newDefenses,
                            p_buildings_new: newBuildings
                        });

                        if (rpcError) {
                            console.error("Mission Outcome RPC Error:", rpcError);
                            // If generic error, we might want to retry, but for valid permissions it should work.
                            if (rpcError.message.includes('UNAUTHORIZED')) {
                                // Should not happen with SECURITY DEFINER unless ownership check fails
                                console.error("Critical Auth Error in Mission RPC");
                            }
                            // Note: No VERSION_MISMATCH retry needed for RPC usually as it works on latest data in SQL
                        }
    ...
    const sendAttack = async (ships: Record<ShipId, number>, coords: { galaxy: number, system: number, position: number }) => {
        // DEBUG: 30 seconds for testing as requested
