...
const ResourceItem: React.FC<{ 
    ...
    <div 
        className={`flex items-center gap-3 min-w-[120px] md:min-w-[150px] px-3 py-2 rounded-xl border border-white/5 ${bgClass} backdrop-blur-md flex-shrink-0 shadow-sm transition-colors`}
    >
        <div className={`size-8 md:size-10 rounded-full flex items-center justify-center shadow-lg ${colorClass}`}>
            <span className="material-symbols-outlined text-white text-sm md:text-lg">{icon}</span>
        </div>
        <div className="flex flex-col justify-center">
            <span className="text-[10px] text-[#929bc9] font-bold uppercase tracking-wider mb-0.5">{name}</span>
            <div className="flex flex-col md:flex-row md:items-baseline md:gap-2">
                <span className={`text-sm md:text-lg font-bold font-mono leading-none ${value < 0 ? 'text-red-500 animate-pulse' : 'text-white'}`}>
